...
class AST:
    def __init__(self, syntaxTree):
        ...
        self.varNumbers = 0 #used for %1, %2, %... in llvm
    ...
    def generate_LLVM(self, ast):
        for child in ast.children:
            if child.node.getRuleName() == "comment":
                self.LLVM += child.children[0].node.getRuleName() + '\n'
    ...
    def parse_expr(self, expr):
        for child in expr.children:
            if child.node.getRuleName() == "variableDeclaration":
                ...
                print(rightMostNode.node.getRuleName())
                self.varNumbers += 1
                if child.children[0].children[0].node.getRuleName() == "int":
                    self.register += f"%{self.varNumbers} = alloca i32, align 4" + '\n'
                    self.store += f"store i32 {rightMostNode.node.getRuleName()}, ptr %{self.varNumbers}, align 4" + '\n'
                elif child.children[0].children[0].node.getRuleName() == "float":
                    self.register += f"%{self.varNumbers} = alloca float, align 4" + '\n'
                    self.store += f"store float {rightMostNode.node.getRuleName()}, ptr %{self.varNumbers}, align 4" + '\n'
                elif child.children[0].children[0].node.getRuleName() == "char":
                    self.register += f"%{self.varNumbers} = alloca i8, align 1" + '\n'
                    self.store += f"store i8 {ord(rightMostNode.node.getRuleName()[1])}, ptr %{self.varNumbers}, align 1" + '\n'
                    ...
                    if child.children[0].children[1].children[0].node.getRuleName() == "int":
                        ...
                        self.register += f"%{self.varNumbers} = alloca i8, align 1" + '\n'
                        self.store += f"store i8 {ord(rightMostNode.node.getRuleName()[1])}, ptr %{self.varNumbers}, align 1" + '\n'
                elif child.children[0] == "pointerWord":#if var is pointer
                    self.register += f"%{self.varNumbers} = alloca ptr, align 8" + '\n'
                    ...
                    if valueOtherPointer in self.store:
                        print("ye boi")
                    self.store += f"store ptr %3, ptr %{self.varNumbers}, align 8" + '\n'


            self.parse_expr(child)

    #find value of variable
    def findValue(self, var, ast):
        for child in ast.children:
            if child.node.getRuleName() == "nameIdentifier":
                if child.children[0].node.getRuleName() == var.node.getRuleName():
                    temp_node = child
                    while True:
                        child = child.parent
                        if child.node.getRuleName() == "variableDefinition":
                            child = get_rightmost_node(child)
                            return child
